 ...
 * Datele vin din data/obiecte.json + data/raioane.json (fetch, generate din content/ via Pages CMS).
 * js/data.js + js/raioane.js raman ca fallback (file:// sau fetch esuat). */
(function(MUZEU_FALLBACK, RAIOANE_FALLBACK){
'use strict';
let MUZEU = MUZEU_FALLBACK, RAIOANE = RAIOANE_FALLBACK;
async function incarcaDate(){
  try{
    const [a, b] = await Promise.all([fetch('data/obiecte.json'), fetch('data/raioane.json')]);
    if(a.ok && b.ok){ MUZEU = await a.json(); RAIOANE = await b.json(); return; }
  }catch(e){}
  if(!MUZEU || !RAIOANE) throw new Error('Lipsesc datele muzeului (data/*.json si fallback js/data.js).');
}
/* ---- texte configurabile din Pages CMS (data/texte.json -> MUZEU.texte) ---- */
let TEXTE = {};
function T(sec, cheie, fb){
  const s = TEXTE && TEXTE[sec];
  const v = s ? s[cheie] : undefined;
  return (v === undefined || v === null || v === '') ? fb : v;
}
function tmpl(str, vars){
  return String(str).replace(/\{(\w+)\}/g, (_, k) => (vars && vars[k] !== undefined && vars[k] !== null) ? vars[k] : '');
}
const NR_RAME = 8;
...
const Sunet = (function(){
  ...
  try{
    ...
    if(v !== null) activ = (v === '1');
    else if(MUZEU && MUZEU.muzeu && MUZEU.muzeu.sunet === false) activ = false;
  }catch(e){}
  function ac(){
    if(!ctx){
  ...
  return {
    ...
    initDinDate(defActiv){ try{ if(localStorage.getItem('muzeu-sunet') === null) activ = !!defActiv; }catch(e){ activ = !!defActiv; } },
    comuta(){
      activ = !activ;
...
function tipRaion(id){
  ...
  if(r && r.tip === 'municipiu') return T('tipuri', 'tip_municipiu', 'Municipiul');
  if(r && r.tip === 'autonomie') return T('tipuri', 'tip_autonomie', 'Unitatea');
  return T('tipuri', 'tip_raion', 'Raionul');
}
function numeRaion(id){ const r = raionInfo(id); return r ? r.nume : T('tipuri', 'fallback_nume_raion', 'Chișinău'); }
...
function randareStepper(){
  stepperEl.innerHTML =
    `<button class="inapoi" id="btn-harta" title="${T('navigare', 'titlu_inapoi', 'Înapoi la harta Moldovei')}">${T('navigare', 'inapoi_harta', '← HARTA')}</button>` +
    MUZEU.sali.map(s =>
  ...
  stepperEl.querySelectorAll('a').forEach(a => {
    a.addEventListener('click', e => { e.preventDefault(); Sunet.pas(); alegeSala(a.dataset.sala, true); });
  });
...
function actualizeazaFooter(){
  ...
  if(chip) chip.textContent = ns ? ns : T('panou_sate', 'toate_satele', 'Toate satele');
  if(bs) bs.title = ns ? tmpl(T('panou_sate', 'template_schimba_satul', 'Schimbi satul ({sat})'), {sat: ns}) : T('panou_sate', 'vezi_satele', 'Vezi satele raionului');
  document.title = ns
    ? tmpl(T('footer', 'template_titlu_pagina_sat', '{sat} · {raion} — Muzeul Școlar'), {sat: ns, raion: numeRaion(raionCurent)})
    : tmpl(T('footer', 'template_titlu_pagina', '{raion} — Muzeul Școlar'), {raion: numeRaion(raionCurent)});
}
...
function randareCartus(){
  ...
  if(nr) nr.textContent = T('cartus_sala', 'prefix_sala', 'SALA ') + roman(i + 1);
...
function umpleModalSala(s, i){
  ...
  if(contor) contor.textContent = inSala.length
    ? (inSala.length === 1 ? T('cartus_sala', 'text_un_obiect', 'Un singur obiect te așteaptă aici:') : tmpl(T('cartus_sala', 'template_mai_multe', '{n} obiecte te așteaptă aici:'), {n: inSala.length}))
    : T('cartus_sala', 'gol_sala', 'Sala e încă goală — prima poveste poate fi a ta.');
  if(acum){
    acum.innerHTML = inSala.length
      ? inSala.map((o, k) => `<li data-obiect="${o.id}" role="button" tabindex="0">${o.titlu}</li>`).join('')
      : `<li class="ms-gol" style="cursor:default;border-style:dashed">${T('cartus_sala', 'adauga_primul', 'Adaugă primul obiect prin butonul de pe hartă')}</li>`;
  }
...
function etichetaFila(i, total){
  return T('carusel', 'prefix_fila', 'FILA') + ' ' + roman(i + 1) + ' ' + T('carusel', 'conector_din', 'DIN') + ' ' + roman(total);
}
function cardHTML(o, i, total, clasa){
  ...
  return `<div class="cadru-card ${clasa}" data-i="${i}" role="button" tabindex="0" aria-label="${o.titlu}, ${etichetaFila(i, total)}">
    <div class="cadru rama-${r}">
      ...
      <div class="paspartu">
        <img src="${poze(o)[0]}" alt="${o.titlu}" loading="lazy" draggable="false" onerror="this.src='https://picsum.photos/seed/${o.id}/600/500'">
...
function randareCarusel(dir){
  ...
  if(!n){
    pista.innerHTML = `<div class="car-gol"><div class="car-gol-paspartu"><p class="car-gol-rand1">${T('carusel', 'gol_titlu', 'Nu există încă obiecte aici.')}</p><p class="car-gol-rand2">${T('carusel', 'gol_subtitlu', 'Alege alt sat sau adaugă primul exponat.')}</p></div></div>`;
    document.getElementById('car-titlu').textContent = '';
    const mc0 = document.getElementById('mob-contor');
    if(mc0) mc0.textContent = T('carusel', 'gol_contor', '—');
...
function actualizeazaBtnDosar(){
  ...
  if(!b) return;
  b.textContent = esteDeschis() ? T('dosar', 'inchide', 'ÎNCHIDE DOSARUL') : T('dosar', 'deschide', 'DESCHIDE DOSARUL');
...
function umpleFereastra(){
  ...
  if(st) st.textContent = (o.perioada || '') + (o.an ? T('dosar', 'conector_an', ' · Anul ') + o.an : '');
  ...
  document.getElementById('f-mini').innerHTML = afis.map((u, k) =>
    `<span class="mini${k === 0 ? ' on' : ''}" data-k="${k}" role="button" tabindex="0" aria-label="${T('dosar', 'prefix_imagine', 'Imagine')} ${k + 1} — ${o.titlu}"><span class="mini-fundal" style="background-image:url('${u}')"></span><img src="${u}" alt="${T('dosar', 'prefix_imagine', 'Imagine')} ${k + 1} — ${o.titlu}" loading="lazy" onerror="this.src='https://picsum.photos/seed/${o.id}/200/150'"></span>`
  ).join('');
  ...
  btnPv.textContent = T('dosar', 'citeste_tot', 'Citește toată descrierea ↓');
  ...
  document.getElementById('f-fisa').innerHTML =
    `<dt>${T('dosar', 'et_material', 'Material')}</dt><dd>${o.material || T('dosar', 'valoare_lipsa', '-')}</dd>` +
    `<dt>${T('dosar', 'et_dimensiuni', 'Dimensiuni')}</dt><dd>${o.dimensiuni || T('dosar', 'valoare_lipsa', '-')}</dd>` +
    `<dt>${T('dosar', 'et_autor', 'Autor')}</dt><dd>${o.autor || T('dosar', 'valoare_lipsa', '-')}</dd>` +
    `<dt>${T('dosar', 'et_locatie', 'Locația obiectului')}</dt><dd>${o.locatie_fizica || T('dosar', 'valoare_lipsa', '-')}</dd>`;
}
function deschide(){
...
function comutaDosarMare(forta){
  ...
  if(b){
    ...
    const et = mare ? T('dosar', 'revino', 'Revino la panoul lateral') : T('dosar', 'mareste', 'Mărește dosarul');
...
function arataVizon(){
  ...
  const o = obiecte[idx];
  img.alt = (o ? o.titlu + ' — ' : '') + T('dosar', 'prefix_imagine', 'Imagine') + ' ' + (idxVizon + 1);
  document.getElementById('vizor-caption').textContent =
    tmpl(T('dosar', 'template_caption', '{titlu} · Imagine {k} din {n}'), {titlu: o ? o.titlu : '', k: idxVizon + 1, n: n});
}
...
function randareSate(){
  ...
  const sate = r.sate || [];
  let html = `<button class="sat ${!satCurent ? 'on' : ''}" data-sat="">${T('panou_sate', 'toate_satele', 'Toate satele')}<span class="sat-nr">${MUZEU.obiecte.filter(o => (o.raion || 'chisinau') === raionCurent).length}</span></button>`;
  html += sate.map(s =>
...
function scrieURL(){
  ...
  let u = pagina + '?raion=' + encodeURIComponent(raionCurent);
  if(satCurent) u += '&sat=' + encodeURIComponent(satCurent);
  u += '#sala=' + salaCurenta;
...
/* ---- alegere sală ---- */
function alegeSala(id, scrieHash){
...
function tipNume(r){
  if(!r) return T('tipuri', 'tip_raion', 'Raionul');
  if(r.tip === 'municipiu') return T('tipuri', 'tip_municipiu', 'Municipiul');
  if(r.tip === 'autonomie') return T('tipuri', 'tip_autonomie', 'Unitatea');
  return T('tipuri', 'tip_raion', 'Raionul');
}
function alegeRaion(id){
  ...
  if(!info || info.stare !== 'pilot'){
    ...
    toastPoarta(tmpl(T('pagina_principala', 'template_mesaj_curand', '{tip} {nume} intră în curând în muzeu.'), {tip: tipNume(info), nume: ((g && g.getAttribute('data-nume')) || id)}));
    return;
  }
  raionCurent = id;
  satCurent = null;
...
function leagaContribuie(){
  ...
  b.addEventListener('click', () => {
    Sunet.hartie();
    toastPoarta(T('pagina_principala', 'mesaj_contribuie', 'Mulțumim! Formularul de contribuire va fi disponibil în curând.'));
  });
}
...
function leagaSunet(){
  ...
  function desen(){
    ...
    if(stare) stare.textContent = Sunet.activ ? T('footer', 'sunet_da', 'SUNET: DA') : T('footer', 'sunet_nu', 'SUNET: NU');
  }
  desen();
...
document.getElementById('f-poveste-btn').addEventListener('click', () => {
  ...
  document.getElementById('f-poveste-btn').textContent = deschis ? T('dosar', 'arata_putin', 'Arată mai puțin ↑') : T('dosar', 'citeste_tot', 'Citește toată descrierea ↓');
  Sunet.hartie();
});
...
/* ---- texte statice din Pages CMS (data/texte.json) aplicate in DOM ---- */
function setText(id, val){ const el = document.getElementById(id); if(el && val !== undefined) el.textContent = val; }
function setAriaTitle(id, val){ const el = document.getElementById(id); if(el && val !== undefined){ el.setAttribute('aria-label', val); el.title = val; } }
function aplicaTexteStatice(){
  document.title = T('pagina_principala', 'titlu_pagina', document.title);
  const meta = document.querySelector('meta[name="description"]');
  if(meta) meta.content = T('pagina_principala', 'descriere_meta', meta.content);
  setText('harta-titlu', T('pagina_principala', 'titlu_harta', 'HARTA MUZEELOR DIN REPUBLICA MOLDOVA'));
  setText('harta-nota', T('pagina_principala', 'subtitlu_harta', 'Alege raionul și intră în muzeu'));
  setText('btn-contribuie', T('pagina_principala', 'text_buton_contribuie', 'ADAUGĂ UN OBIECT ÎN COLECȚIA NOASTRĂ VIRTUALĂ'));
  setText('sate-eticheta', T('panou_sate', 'eticheta_selecteaza', 'SELECTEAZĂ SATUL'));
  setAriaTitle('sate-inchide', T('panou_sate', 'inchide_lista', 'Închide lista'));
  setText('ms-eticheta', T('cartus_sala', 'antet_muzeu', 'MUZEUL ȘCOLAR'));
  setText('ms-tipice-titlu', T('cartus_sala', 'titlu_tipice', 'CE OBIECTE POT FI AICI'));
  setText('ms-acum-titlu', T('cartus_sala', 'titlu_acum', 'ÎN SALĂ ACUM'));
  setAriaTitle('modal-sala-inchide', T('panou_sate', 'inchide_lista', 'Închide lista'));
  setText('cartus-detalii', T('cartus_sala', 'eticheta_detalii', 'ⓘ DETALII'));
  setAriaTitle('f-inchide', T('dosar', 'inchide_titlu', 'Sigilează dosarul'));
  document.getElementById('f-inchide')?.setAttribute('aria-label', T('dosar', 'inchide_aria', 'Închide dosarul'));
  setAriaTitle('f-mareste', T('dosar', 'mareste', 'Mărește dosarul'));
  const mare = document.getElementById('f-mare');
  if(mare && !mare.src) mare.alt = T('dosar', 'alt_obiect', 'Imagine obiect');
  setAriaTitle('f-mare-cadru', T('dosar', 'hint_fullscreen', 'Deschide imaginea fullscreen'));
  setText('f-prev', T('dosar', 'fila_anterioara', '← Fila anterioară'));
  setText('f-next', T('dosar', 'fila_urmatoare', 'Fila următoare →'));
  setAriaTitle('vizor-inchide', T('dosar', 'vizor_inchide', 'Închide imaginea'));
  setAriaTitle('vizor-prev', T('dosar', 'vizor_anterioara', 'Imaginea anterioară'));
  setAriaTitle('vizor-next', T('dosar', 'vizor_urmatoare', 'Imaginea următoare'));
  const vimg = document.getElementById('vizor-img');
  if(vimg && !vimg.src) vimg.alt = T('dosar', 'alt_marita', 'Imagine mărită');
  const bsunet = document.getElementById('btn-sunet');
  if(bsunet) bsunet.title = T('footer', 'titlu_buton_sunet', 'Sunet arhivă pornit/oprit');
  setText('footer-semnatura', T('footer', 'semnatura', 'arhiva Bou Elena'));
  const fr0 = document.getElementById('f-raion');
  if(fr0 && !fr0.textContent) fr0.textContent = T('footer', 'nume_implicit', 'Muzeul Liceului Alexandru cel Bun');
}

/* ---- pornire + deep-link (?raion= &sat= / ?obiect= / ?sala= / #sala=) ---- */
(async function start(){
  try{ await incarcaDate(); }
  catch(e){
    const p = document.getElementById('car-pista');
    if(p) p.innerHTML = `<div class="car-gol"><div class="car-gol-paspartu"><p class="car-gol-rand1">${T('erori', 'titlu_date', 'Nu s-au putut încărca datele muzeului.')}</p><p class="car-gol-rand2">${T('erori', 'subtitlu_date', 'Verifică data/obiecte.json și data/raioane.json.')}</p></div></div>`;
    throw e;
  }
  DATE_RAIOANE = (RAIOANE && RAIOANE.raioane) ? RAIOANE.raioane : [];
  salaCurenta = (MUZEU.sali[0] && MUZEU.sali[0].id) || 'sala-documentelor';
  TEXTE = (MUZEU && MUZEU.texte) || {};
  Sunet.initDinDate(!(MUZEU && MUZEU.muzeu && MUZEU.muzeu.sunet === false));
  aplicaTexteStatice();
  leagaSunet();
  actualizeazaBtnDosar();
  const q = new URLSearchParams(location.search);
  const h = (location.hash.match(/sala=([\w-]+)/) || [])[1];
  /* raion din URL sau implicit pilotul */
  const rq = q.get('raion');
  raionCurent = (rq && raionInfo(rq)) ? rq : 'chisinau';
  const sq = q.get('sat');
  satCurent = (sq && numeSat(raionCurent, sq)) ? sq : null;
  randareSate();
  actualizeazaFooter();
  const idObiect = q.get('obiect');
  if(idObiect){
    const o = MUZEU.obiecte.find(x => x.id === idObiect);
    if(o){
      raionCurent = o.raion || raionCurent;
      satCurent = o.sat || null;
      alegeSala(o.sala, false);
      idx = Math.max(0, obiecte.findIndex(x => x.id === idObiect));
      randareCarusel();
      randareSate();
      actualizeazaFooter();
      scrieURL();
      leagaPoarta(true);
      setTimeout(deschide, 350);
      return;
    }
  }
  alegeSala(q.get('sala') || h || MUZEU.sali[0].id, false);
  randareSate();
  actualizeazaFooter();
  leagaPoarta(false);
})();
})(typeof MUZEU !== 'undefined' ? MUZEU : null, typeof RAIOANE !== 'undefined' ? RAIOANE : null);
